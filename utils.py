...
def load_LFdata(dir_LFimages):
    ...
    for dir_LFimage in dir_LFimages:
        ...
        for i in range(81):
            try:
                tmp = np.float32(
                    imageio.imread(dir_LFimage + '/input_Cam0%.2d.png' % i))  # load LF images(9x9)
            except:
                print(dir_LFimage + '/input_Cam0%.2d.png..does not exist' % i)
            traindata_all[image_id, :, :, i // 9, i - 9 * (i // 9), :] = tmp
            del tmp
        try:
            tmp = np.float32(read_pfm( dir_LFimage + '/gt_disp_lowres.pfm'))  # load LF disparity map
        except:
            print( dir_LFimage + '/gt_disp_lowres.pfm..does not exist' % i)
